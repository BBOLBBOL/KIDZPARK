...
<style>
    ...
    .table {
        ...
        max-width: 7em;             /* 최대 너비 설정 (10em은 예시) */
    }
</style>
</head>
			...
			<table class="table table-hover">
				...
				<c:forEach var="notice" items="${getnoticelist}">
						<tr style="background-color: #ffebee;">
							...
							<td class="name"><a href="/BoardView?b_idx=${notice.b_idx}&u_no=${notice.u_no}&m_no=${m_no}">${notice.b_title}</a></td>
					...
					<c:forEach var="board" items="${getboardlist}">
						<tr>
							<td><a href="/BoardView?b_idx=${board.b_idx}&u_no=${board.u_no}&m_no=${m_no}" >${board.b_like}</a></td>
							<td class="name"><a href="/BoardView?b_idx=${board.b_idx}&u_no=${board.u_no}&m_no=${m_no}">${board.b_title}</a></td>
							<td><a href="/BoardView?b_idx=${board.b_idx}&u_no=${board.u_no}&m_no=${m_no}">${board.u_nickname}</a></td>
							<td><a href="/BoardView?b_idx=${board.b_idx}&u_no=${board.u_no}&m_no=${m_no}" >${board.b_writedate}</a></td>
							<td><a href="/BoardView?b_idx=${board.b_idx}&u_no=${board.u_no}&m_no=${m_no}" >${board.b_readcount}</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
