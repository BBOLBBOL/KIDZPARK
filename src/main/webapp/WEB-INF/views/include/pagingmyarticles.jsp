  ...
  <div>
    <table width="500" height="25"  align="center"  >
     ...
     <c:if test="${ pg.startPage gt 1 }">
       <a href="/MyArticles?m_no=${m_no}&u_no=${loginVo.u_no}&nowPage=1">처음</a>
       <a href="/MyArticles?m_no=${m_no}&u_no=${loginVo.u_no}&nowPage=${pg.startPage - 1}">
       [이전]
       </a>
     </c:if>
    ...
    <c:choose>
        ...
        <c:otherwise>
            <a href="/MyArticles?m_no=${m_no}&u_no=${loginVo.u_no}&nowPage=${pageNumber}">
                ${pageNumber}
            </a>&nbsp;&nbsp;
        </c:otherwise>
    </c:choose>
     ...
     <c:if test="${ pg.nowPage ne endPage }">
       <a href="/MyArticles?m_no=${m_no}&u_no=${loginVo.u_no}&nowPage=${ pg.nowPage + 1 }">
       [다음]
       </a>
       <a href="/MyArticles?m_no=${m_no}&u_no=${loginVo.u_no}&nowPage=${pg.lastPage}">마지막</a>
     </c:if>    
