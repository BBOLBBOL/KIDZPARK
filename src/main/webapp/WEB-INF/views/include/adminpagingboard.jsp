  ...
  <div>
    <table width="500" height="25"  align="center"  >
     ...
     <c:if test="${ pg.startPage gt 1 }">
       <a href="/NoticeList?m_no=${m_no}&nowPage=1">처음</a>
       <a href="/NoticeList?m_no=${m_no}&nowPage=${pg.startPage - 1}">
       [이전]
       </a>
     </c:if>
    ...
    <c:choose>
        ...
        <c:otherwise>
            <a href="/NoticeList?m_no=${m_no}&nowPage=${pageNumber}">
                ${pageNumber}
            </a>&nbsp;&nbsp;
        </c:otherwise>
    </c:choose>
     ...
     <c:if test="${ pg.nowPage ne endPage }">
       <a href="/NoticeList?m_no=${m_no}&nowPage=${ pg.nowPage + 1 }">
       [다음]
       </a>
       <a href="/NoticeList?m_no=${m_no}&nowPage=${pg.lastPage}">마지막</a>
     </c:if>    
