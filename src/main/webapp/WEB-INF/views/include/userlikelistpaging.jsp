...
<%@taglib  prefix="c"  uri="http://java.sun.com/jsp/jstl/core" %>
  ...
  <c:set  var="u_no"                value="${loginVo.u_no}" />
  ...
  <div>
    <table width="500" height="25"  align="center"  >
     ...
     <c:if test="${ pds.startPage gt 1 }">
       <a href="/UserLikeList?u_no=${u_no }&nowPage=1">처음</a>
       <a href="/UserLikeList?u_no=${u_no }&nowPage=${pds.startPage - 1}">
       [이전]
       </a>
     </c:if>
    ...
    <c:choose>
        ...
        <c:otherwise>
            <a href="/UserLikeList?u_no=${u_no }&nowPage=${pageNumber}">
                ${pageNumber}
            </a>&nbsp;&nbsp;
        </c:otherwise>
    </c:choose>
     ...
     <c:if test="${ pds.nowPage ne endPage }">
       <a href="/UserLikeList?u_no=${u_no }&nowPage=${ pds.nowPage + 1 }">
       [다음]
       </a>
       <a href="/UserLikeList?u_no=${u_no }&nowPage=${pds.lastPage}">마지막</a>
     </c:if>    
